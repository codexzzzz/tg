...
TOKEN = "8610520965:AAE6D_7DjtIcgazvThuFL-sG28A7pme7SRQ"
PREMIUM_STARS = 10
...
async def chan_start_cb(call: types.CallbackQuery):
    ...
    if discussion_id:
        ...
        await call.answer("✅ Снос запускается...")
        await call.message.answer(
            f"🔍 Ищу последний пост в *{info['title']}*...",
            parse_mode="Markdown",
        )

        latest_post_id = await _find_latest_post_id(ch_id, discussion_id)

        is_turbo = turbo_mode.get(uid, False) and uid in premium_users

        if latest_post_id:
            if is_turbo:
                task = asyncio.create_task(turbo_spam_task(uid, discussion_id, reply_to=latest_post_id, thread_id=latest_post_id))
                mode_label = f"🚀 ТУРБО ({TURBO_BURST_SIZE} сообщ. каждые {TURBO_PAUSE}с)"
            else:
                task = asyncio.create_task(spam_task(uid, discussion_id, delay, reply_to=latest_post_id, thread_id=latest_post_id))
                mode_label = f"задержка: {delay} сек."
            spamming[uid][ch_id] = task
            await call.message.answer(
                f"🚀 Снос запущен под постом *#{latest_post_id}* в *{info['title']}* (комментарии, {mode_label})",
                parse_mode="Markdown",
            )
        else:
            if is_turbo:
                task = asyncio.create_task(turbo_spam_task(uid, discussion_id))
                mode_label = f"🚀 ТУРБО ({TURBO_BURST_SIZE} сообщ. каждые {TURBO_PAUSE}с)"
            else:
                task = asyncio.create_task(spam_task(uid, discussion_id, delay))
                mode_label = f"задержка: {delay} сек."
            spamming[uid][ch_id] = task
            await call.message.answer(
                f"⚠️ Не нашёл активный пост, спам идёт в группу обсуждений *{info['title']}* ({mode_label})",
                parse_mode="Markdown",
            )
        return
    else:
    ...
    await call.message.answer(
        f"🚀 Снос запущен в *{info['title']}* (канал, {mode_label})",
        parse_mode="Markdown",
    )
...
# --- 📨 Спам поста (авто-определение последнего поста) ---
...
async def chan_post_cb(call: types.CallbackQuery):
    ...
    await call.answer("🔍 Ищу последний пост...")
    await call.message.answer(
        f"🔍 Ищу последний пост в *{info['title']}*...",
        parse_mode="Markdown",
    )
    ...
    if not latest_post_id:
        await call.message.answer(
            "❌ Не удалось найти активный пост в канале.\n"
            "Убедись, что в канале есть посты с включёнными комментариями.",
        )
        return
    ...
    spamming[uid][ch_id] = task
    await call.message.answer(
        f"🚀 Спам под постом *#{latest_post_id}* запущен!\n"
        f"Канал: *{info['title']}*\n"
        f"Режим: {mode_label}\n\n"
...
async def chan_all_posts_cb(call: types.CallbackQuery):
    ...
    delay = user_cd.get(uid, 1.0)
    task = asyncio.create_task(all_posts_spam_task(uid, ch_id, discussion_id, delay))
    spamming[uid][ch_id] = task
...
async def _detect_channel_max_post_id(ch_id: int, discussion_id: int) -> int:
    """Определяет примерный ID последнего поста в канале."""
    # Лучший способ: отправить в сам канал (если бот-администратор)
    try:
        temp = await bot.send_message(ch_id, ".")
        max_id = temp.message_id
        await bot.delete_message(ch_id, temp.message_id)
        logging.info(f"Channel max post ID detected from channel: {max_id}")
        return max_id
    except Exception:
        pass

    # Если бот не админ в канале — смотрим на закреплённое сообщение
    try:
        chat = await bot.get_chat(ch_id)
        pinned = getattr(chat, "pinned_message", None)
        if pinned and pinned.message_id:
            logging.info(f"Channel max post ID from pinned: {pinned.message_id}")
            return pinned.message_id + 100
    except Exception:
        pass

    # Fallback: определяем макс ID группы обсуждений и берём меньшее значение
    try:
        temp = await bot.send_message(discussion_id, ".")
        disc_max = temp.message_id - 1
        await bot.delete_message(discussion_id, temp.message_id)
        # Посты канала = низкие ID, ответы = высокие ID
        # Берём мин(disc_max, 1000) чтобы охватить типичные каналы
        result = min(disc_max, 1000)
        logging.info(f"Channel max post ID fallback: {result}")
        return result
    except Exception:
        return 500


async def _find_latest_post_id(ch_id: int, discussion_id: int) -> int | None:
    """Находит ID последнего активного поста канала в группе обсуждений."""
    max_post_id = await _detect_channel_max_post_id(ch_id, discussion_id)
    probe_end = max(1, max_post_id - 500)
    for post_id in range(max_post_id, probe_end - 1, -1):
        try:
            ...
            await bot.delete_message(discussion_id, sent.message_id)
            logging.info(f"Found latest post ID: {post_id}")
            return post_id
        except asyncio.CancelledError:
            raise
        except Exception:
...
async def all_posts_spam_task(uid: int, ch_id: int, discussion_id: int, delay: float):
    # Определяем верхний предел из канала (не из группы обсуждений!)
    try:
    ...
    # Проверяем все посты от последнего вниз (до 500 назад для охвата всех постов)
    valid_ids = []
    probe_end = max(1, max_post_id - 499)
